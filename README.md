...
2. **כתיבה** — כל שחקן מקבל תבנית מם אקראית וכותב עליה כיתוב:
   - עד 5 שורות, לא רק עליונה ותחתונה — מוסיפים ומוחקים שורות בכפתור.
   - כל שורה יושבת איפה שרוצים **בתוך התמונה**: גוררים אותה על התצוגה
     המקדימה (עובד גם במגע).
   - לא אהבתם את התמונה? **מחליפים** אותה (עד 20 החלפות בסיבוב, לפי הגדרת
     המארח) — הטקסט שכתבתם נשמר.
   - יש תצוגה מקדימה חיה, והטקסט נשמר אוטומטית גם בלי ללחוץ "שלחו מם".
...
- עורך כיתוב חופשי: עד 5 שורות, מיקום חופשי בתוך התמונה בגרירה, והחלפת תבנית.
...
- חיבור מחדש — מי שהתנתק באמצע משחק שומר את המקום והניקוד וחוזר עם אותו קישור.
  רענון או נעילת מסך מחזירים ישר למשחק; חזרה אחרי יותר מ‑5 דקות שואלת קודם
  "לחזור למשחק או לפתוח חדש?" במקום לגרור אתכם למשחק ישן.
- יציאה מהחדר זמינה מכל מסך — לוחצים על קוד החדר בכותרת.
...
הגדרות המשחק (סיבובים, זמן כתיבה, זמן דירוג, מספר החלפות תמונה לסיבוב)
נקבעות בלובי על ידי המארח.
...
| `meme:draft`, `meme:submit` | שמירת טיוטה / שליחת מם (רשימת שורות עם מיקום) |
| `meme:swap` | החלפת התבנית שקיבלתם |
| `vote:cast` | דירוג 1–5 |
